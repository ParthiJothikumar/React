...
def _flush_trace(agent_calls, trace, user_id, conv_id) -> None:
    """Write the turn's agent_calls rows. Called from a finally, AFTER the transaction.

    Two rules, both deliberate:

    AFTER the transaction() block, never inside it. AgentCallRepository shares this
    request's connection, so its commit is that connection's commit -- called inside an
    open transaction it would commit the turn's half-written work early and defeat the
    all-or-nothing boundary. Once the block has exited there is nothing in flight for it
    to catch.

    In a finally, so it also runs when the turn FAILED. That is when the trace matters
    most: it holds the timing and the error of the call that broke the turn, and if it
    were tied to the rolled-back transaction that evidence would be discarded with it.
    """
    if agent_calls is None or trace is None or not trace.rows:
        return
    agent_calls.append_calls(user_id, conv_id, trace.rows)
    trace.rows.clear()  # so a retry on the same object cannot double-write
...
class ChatService:
    ...
    def __init__(
        ...
        # Optional so the existing tests can build a service without them.
    ...
    def start_chat(self, user_id: str, message: str) -> dict:
        ...
        try:
            ...
            _flush_trace(
                self._agent_calls, self._trace, user_id, conversation.id
            )
    ...
    def continue_chat(self, user_id: str, session_id: str, message: str) -> dict:
        ...
        try:
            ...
            _flush_trace(self._agent_calls, self._trace, user_id, conv_id)
...
class JobService:
    ...
    def advance_job(
        ...
        """
        try:
            return self._advance_job(user_id, conversation_id, session_id)
        finally:
            # After the poll's own writes, so the trace is never inside their
            # transaction; in a finally, so a failed poll still records why.
            _flush_trace(self._agent_calls, self._trace, user_id, conversation_id)
