...
def step(conv_id: str, user_message: str, stage, flow_vars: dict):
    ...
    elif stage == AWAITING_FOLLOWUP:
        result = _followup_turn(conv_id, user_message, flow_vars, messages)
...
def _run_second_classification(conv_id, flow_vars, messages):
    ...
    Sends {conversation_id, kb_id, message} and hands the ValidationResponse to
    _route_second_classification, which drives every branch of the decision tree.
    ask_user follow-up replies come back in through _followup_turn.
    """
    second_classification = call_second_classification(
        conv_id, flow_vars.get("kb_id"), message=""
    )
    return _route_second_classification(conv_id, flow_vars, messages, second_classification)


def _followup_turn(conv_id, user_message, flow_vars, messages):
...
def _route_second_classification(conv_id, flow_vars, messages, second_classification):
    """Route on one ValidationResponse from the SECOND classification agent.

    `second_classification` is the agent's decision dict (the M/A/U/N scenarios).
    Order matters: success and validated are checked first, so every failure or
    non-automatable result (M2/U5/U6/N1/N2) short-circuits to _close_with_message
    before any action / user_params / follup_flag branch is considered.
    """
    # success == false (M2/U5/U6/N2, or a synthesized transport failure) -> close.
    if not second_classification.get("success"):
        return _close_with_message(
            conv_id, flow_vars, messages, second_classification, log_error=True
        )

    # success but not automatable (N1: e.g. not found) -> show message, close. error null.
    if not second_classification.get("validated"):
        return _close_with_message(
            conv_id, flow_vars, messages, second_classification, log_error=False
        )

    action = (second_classification.get("action") or "").lower()

    # MANUAL (M1): steps already embed the reference link -> show + ask confirmation.
    if action == ACTION_MANUAL:
        messages.append(
            second_classification.get("steps")
            or second_classification.get("agent_message")
            or ""
        )
        messages.append("Did these steps resolve your issue?")
        return messages, AWAITING_RESOLVED, flow_vars

    # Drift guard: anything but a clean "automatic" -> route to the team (never auto-run).
    if action != ACTION_AUTOMATIC:
        logger.warning(
            "second classification returned unexpected action=%r conv_id=%s; "
            "routing to support team",
            second_classification.get("action"), conv_id,
        )
        return _close_with_message(
            conv_id, flow_vars, messages, second_classification, log_error=False
        )
    ...
    if not second_classification.get("user_params"):
        return _start_diagnostics(conv_id, flow_vars, messages)

    # AUTOMATIC + ask_user, still gathering input (U1/U2/U4) -> ask and wait. The agent
    # owns the follow-up count and returns success=false when it hits the cap (U5).
    if second_classification.get("follup_flag"):
        if second_classification.get("follow_up_question"):
            messages.append(second_classification["follow_up_question"])
        return messages, AWAITING_FOLLOWUP, flow_vars

    # AUTOMATIC + ask_user, inputs ready (U3) -> troubleshoot directly with the script.
    return _start_troubleshoot(
        conv_id, flow_vars, messages,
        second_classification.get("script_name"),
        second_classification.get("script_params"),
    )


def _close_with_message(conv_id, flow_vars, messages, second_classification, log_error):
    ...
    """
    raw_error = second_classification.get("error")
    if log_error and raw_error:
        logger.error(
            "second-classification failed conv_id=%s stop=%s: %s",
            conv_id, second_classification.get("stop_reason"), raw_error,
        )
        flow_vars["last_error"] = raw_error
    messages.append(second_classification.get("agent_message") or SECOND_CLASS_FALLBACK)
    snow_update_incident(
...
def _start_diagnostics(conv_id, flow_vars, messages):
    """A1 path: START an async diagnostics job instead of blocking.

    We only record the intent in flow_vars["start_job"]; routes.py (after step())
...
def _start_troubleshoot(conv_id, flow_vars, messages, script_name, script_params):
    """U3 path: skip diagnostics and START the troubleshoot job with the collected
    script. Same start_job mechanism as _start_diagnostics; the job input carries the
    script_name + script_params the ask_user loop gathered.
    """
    snow_update_incident(
