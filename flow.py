            ...
            "input": json.dumps(
                {
                    ...
                    # Which machine to remediate. Currently a single configured test
                    # device for every user -- see Settings.DEFAULT_DEVICE_ID.
    ...
    def _proceed_turn(self, conv_id, user_message, flow_vars, messages):
        ...
        if "yes" in user_message.lower():
            ...
            flow_vars["start_job"] = {
                ...
                "start_url": self._settings.TROUBLESHOOT_START_URL,
                "status_url": self._settings.TROUBLESHOOT_STATUS_URL,
                # JSON, like the other two start_job payloads. This one used to send the
                # bare summary string, which the receiving service could not json.loads()
                # -- so every troubleshoot started from "yes, proceed" would have failed
                # at the far end while the other two paths worked.
                "input": json.dumps(
                    {
                        "kb_id": flow_vars.get("kb_id"),
                        "summary": flow_vars.get("kb_summary", ""),
