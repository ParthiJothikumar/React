...
def start_job_record(conn, user_id: str, conv_id: str, job_id: str,
                     baseline: str = None) -> None:
    """Mark the conversation's job as just-started (running).

    `baseline` is the trigger-time timestamp (see jobs_agent.job_baseline_stamp);
    ...
    conn.cursor().execute(
        ...
        "job_result = NULL, job_baseline = ?, updated_at = ? WHERE user_id = ? AND id = ?",
        (job_id, JOB_RUNNING, "Starting...", baseline, _now(), user_id, conv_id),
    )
    conn.commit()
...
def get_job(conn, user_id: str, conv_id: str) -> dict:
    """Return {job_id, job_status, job_progress, job_result, job_baseline}."""
    cur = conn.cursor()
    cur.execute(
        "SELECT job_id, job_status, job_progress, job_result, job_baseline "
