...
class ServiceNowClient:
    ...
    def create_incident(self, conv_id: str, interaction_id, issue_type, details) -> dict:
        ...
        NOT SENT: a `correlation_id` for ServiceNow-side deduplication. It was tried and
        removed deliberately -- see the note in flow.py. The short version: it only pays
        off if ServiceNow matches OPEN incidents only, and a wrong merge (a new problem
        attached to a closed ticket) is worse than the duplicate it prevents.
        """
        return self._post(
            conv_id,
            {
