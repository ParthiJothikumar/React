...
from app.clients.jobs_agent import check_status, start_job
...
def _dispatch_pending_job(conn, user_id, conv_id, pending):
    ...
    start_job_record(conn, user_id, conv_id, job_id)
...
def job_status(user_id: str, conversation_id: str, session_id: Optional[str] = None):
    ...
    try:
        ...
        for _ in range(JOB_STATUS_RETRIES):
            try:
                status = check_status(kind, status_url, job.get("job_id"), polls)  # LIVE call
                break
            except Exception:
