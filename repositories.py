...
class AgentCallRepository(BaseRepository):
    """agent_calls: the internal trace of every agent Function App call.
    ...
    Its own commit, NOT the caller's transaction. Every other write in this file defers
    to transaction() so a turn commits once. This one must not: if the turn rolls back,
    the trace of the call that CAUSED the rollback would be rolled back with it, which
    destroys exactly the evidence the table exists to keep.
    ...
    which is when the trace matters most.
    """
