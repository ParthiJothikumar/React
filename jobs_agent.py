...
class JobsClient(HttpClient):
    ...
    # -- status -------------------------------------------------------------
    def check_status(
        self, kind: str, status_url: str, job_id: str, tries: int, baseline: str = ""
    ) -> dict:
        """Ask the agent how the job is going. Returns {state, message, output}.

        `tries` is how many times we've already checked -- carried for the caller's poll
        accounting and for a scripted test stand-in; the real path ignores it. `baseline`
        is the trigger-time timestamp (from conversations.job_baseline) used to tell OUR
        ...
        if "detectionState" in data or "remediationState" in data:
            return self._derive_intune(data, baseline)
